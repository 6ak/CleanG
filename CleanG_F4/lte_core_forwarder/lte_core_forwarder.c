...
static struct rte_hash *encap_hash;
...
packet_handler(struct rte_mbuf* pkt, struct onvm_pkt_meta* meta) {
  ...
  //  if (pkt->pkt_len > 600)
  //  {
  //    printf ("alli! there is a packet larger than 600!\n");
  /*    FILE *fp;
	fp = fopen("dump.txt", "a");
	if (pkt->next != NULL) {
  ...
  if ((eh->d_addr.addr_bytes[0] == 0x8c) && (eh->d_addr.addr_bytes[1] == 0xdc)
      && (eh->d_addr.addr_bytes[2] == 0xd4) && (eh->d_addr.addr_bytes[3] == 0xac)
      && (eh->d_addr.addr_bytes[4] == 0x6b) && (eh->d_addr.addr_bytes[5] == 0x21) )       
  {
  ...
  if (pkt->port == DATA_COMMAND_PORT) {
    ...
    struct CoreDataPathSet *c = (struct CoreDataPathSet*) rte_ctrlmbuf_data(pkt);
...
#if UTILIZATION_LOGGING == ACTIVATED
    recordUtilizationLog ("ULogf4forwarder.txt", utilization);
#endif
