...
static uint64_t datapath_id = 1875;
//static uint16_t packet_id = 0;
extern void sendLTE5Response( struct SgwOpLte5* op5);
extern void sendLTE18Response( struct SgwOpLte18* op18);
extern void sendLTE28Response( struct SgwOpLte28* op28);
extern void sendLTED7Response (struct GwOpPlaceHolder* ph);
extern void sendLTEI16Response (struct GwOpPlaceHolder* ph);
extern void sendLTEA3Response (struct GwOpPlaceHolder* ph);
extern void sendLTEH22Response (struct GwOpPlaceHolder* ph);
extern void sendLTEH18Response (struct GwOpPlaceHolder* ph);


static uint32_t current_xid = 2000;
...
int handleOfPackets (struct rte_mbuf* pkt, struct onvm_pkt_meta* meta) {
  ali_debug_pprint2 ("start of handling\n");
  struct ether_hdr * eh;
  ...
  if (eh->ether_type == rte_be_to_cpu_16(ETHER_TYPE_IPv4)){
    ...
    struct ipv4_hdr *iph;
    ali_debug_pprint2 ("before iph mtod\n");
    iph = rte_pktmbuf_mtod (pkt,struct ipv4_hdr*);
    if (((iph->fragment_offset) & (0b0010000000000000)) != 0)
    {
      critical_print(" Ali this packet is fragmented!!! %u\n", iph->fragment_offset);
    }
	...
	if (iph->next_proto_id == IPPROTO_TCP) {
	  ali_debug_pprint2("in of tcp\n");
	  rte_pktmbuf_adj(pkt, 20);
	  struct tcp_hdr *tcp;
	  tcp = rte_pktmbuf_mtod (pkt,struct tcp_hdr*);
	  if (tcp->src_port != rte_be_to_cpu_16(6653) || rte_be_to_cpu_16(tcp->dst_port) != our_port) {
	    critical_pprint (" a packet is comign from other ports!! \n");
	  }
	  uint32_t tcp_header_size = ((tcp->data_off) >> 4) * 4;
	  uint32_t l4_len = (tcp->data_off & 0xf0) >> 2;
	  if (tcp_header_size != l4_len) {
	    critical_print ("something is wrong with calculation of tcp header size!! mine %u l4 is %u\n", tcp_header_size, l4_len);
	  }
	  ...
	  if (tcp->tcp_flags == 18) {
	    ...
	    ali_debug_pprint2("first stop in of packet handling \n");
	    if (remainded_size != 0 && packet_data_size != 0)
	    {
	      //printf ("saved data used!\n");
	      ali_debug_print2 ("before adding previous packet, ramindedsize is %u, pkt size is %u\n", remainded_size, packet_data_size);
	      char * packet_begin = rte_pktmbuf_prepend( pkt, remainded_size);
	      if (packet_begin == NULL) {
		printf ("could not extend packet!\n");
		rte_exit(1,"could not extend packet!\n");
	      } else {
	    ...
	    //int loopBugFinder = 0;
	    ali_debug_pprint2 ("before the while\n");
	    while (packet_data_size > processed_openflow_data) {
	      //loopBugFinder++;
	      ofph_in = rte_pktmbuf_mtod (pkt,struct ofp_header*);
	      //printf ("version is %u\n", ofph_in->version);
	      if (ofph_in->version != 1) {
		printf ("somethine is seriously wrong! we are processing non sense!\n");
		FILE *fp;
		  fp = fopen("nonesensepacket.txt", "a");
		fprintf (fp, "pkt len is %u, pkt data len %u packet data size is %u, processed is %u", pkt->pkt_len, pkt->data_len, packet_data_size, processed_openflow_data);
		fprintf (fp, "packet data size is %u and processed is %u\n", packet_data_size, processed_openflow_data);
		rte_pktmbuf_dump(fp, pkt, packet_data_size);
		fclose(fp);
	      }
	      //TODO: ali find out why the following conditions are acutally happening
	      /*
		...
		remainded_size = packet_data_size - processed_openflow_data;
		//printf ("saving data for later, size is %u \n", remainded_size);
		ali_debug_pprint2 ("before mem copy\n");
		memcpy (remainder_data, ofph_in,remainded_size);
		ali_debug_pprint2("after mem copy\n");
		break;
	      }

	      processed_openflow_data += rte_be_to_cpu_16 (ofph_in->length);
	      ali_debug_pprint2 ("just before switch\n");
	      switch (ofph_in->type) {
		case OFPT_HELLO:
		...
		case OFPT_PACKET_OUT:
		  ...
		  if (*messageCode2 == SGW_OP_LTE5_BACK_CODE){
		    ali_debug_pprint ("op lte5 is received\n");
		    struct SgwOpLte5 *op5 = rte_pktmbuf_mtod_offset (pkt, struct SgwOpLte5*, 26);
		    sendLTE5Response(op5);
		  } else if (*messageCode2 == SGW_OP_LTE18_BACK_CODE){
		    ali_debug_pprint ("op lte 18 is received\n");
		    struct SgwOpLte18 *op18 = rte_pktmbuf_mtod_offset (pkt, struct SgwOpLte18*, 26);
		    if (op18->imsi > USER_STATE_SIZE)
		      critical_print ("large imsi! %lu \n", op18->imsi);
		    sendLTE18Response(op18);
		  } else if (*messageCode2 == SGW_OP_LTE28_BACK_CODE){
		    ali_debug_pprint ("op lte 28 is received\n");
		    struct SgwOpLte28 *op28 = rte_pktmbuf_mtod_offset (pkt, struct SgwOpLte28*, 26);
		    sendLTE28Response(op28);
		  } else if (*messageCode2 == SGW_OP_DETACH_BACK_CODE){
		    ali_debug_pprint ("op DETACH is received\n");
		    struct GwOpPlaceHolder *ph = rte_pktmbuf_mtod_offset (pkt, struct GwOpPlaceHolder*, 26);
		    sendLTED7Response(ph);
		  } else if (*messageCode2 == SGW_OP_ITOA_BACK_CODE){
		    ali_debug_pprint ("op itoa is received\n");
		    struct GwOpPlaceHolder *ph = rte_pktmbuf_mtod_offset (pkt, struct GwOpPlaceHolder*, 26);
		    sendLTEI16Response(ph);
		  } else if (*messageCode2 == SGW_OP_ATOI_BACK_CODE){
		    ali_debug_pprint ("op atoi is received\n");
		    struct GwOpPlaceHolder *ph = rte_pktmbuf_mtod_offset (pkt, struct GwOpPlaceHolder*, 26);
		    sendLTEA3Response(ph);
		  } else if (*messageCode2 == SGW_OP_H22_BACK_CODE){
		    ali_debug_pprint ("op h22 is received\n");
		    struct GwOpPlaceHolder *ph = rte_pktmbuf_mtod_offset (pkt, struct GwOpPlaceHolder*, 26);
		    sendLTEH22Response(ph);
		  } else if (*messageCode2 == SGW_OP_H18_BACK_CODE){
		    ali_debug_pprint ("op h18 is received\n");
		    struct GwOpPlaceHolder *ph = rte_pktmbuf_mtod_offset (pkt, struct GwOpPlaceHolder*, 26);
		    sendLTEH18Response(ph);
		  } else {
		    ali_debug_pprint ("unknwon packet out received!!!\n");
		  }
		...
		default:
		  ...
		  printf ("unknown openflow message, type is %u\n", ofph_in->type);
...
void sendPacketIn( void* data_to_send, uint16_t data_size) {
  ...
  struct ofp_packet_in* opi;
  opi = (struct ofp_packet_in*) rte_pktmbuf_append(pkt2, sizeof (struct ofp_packet_in));
  //printf ("ofp packet in size is(should be 20) %lu\n", sizeof (struct ofp_packet_in));
  ofh = &(opi->header);
  ofh->version = OFP_VERSION;
  ofh->type = OFPT_PACKET_IN;
  ofh->length = rte_be_to_cpu_16(packet_in_total_size);
